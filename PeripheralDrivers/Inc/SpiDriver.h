...
typedef struct
{
	...
	uint8_t SPI_baudrate;			// Define la velocidad que maneja el SPI con respecto al Reloj
	uint8_t SPI_fullDupplexEnable;	// Configura si solo recepción o bi-direccional
	uint8_t SPI_frameFormat;		// COnfiguramos si deseamos primero el MSB o el LSB
...
void spi_selectSlave(SPI_Handler_t* ptrHnadlerSPI);
